...
-- Create the function with proper subscription plan handling
...
BEGIN
    ...
    available_plans AS (
        ...
        UNION
        SELECT 'free'::text  -- Always include free tier
    ),
    -- Count users for each available plan
    tier_counts AS (
        SELECT jsonb_object_agg(
            COALESCE(ap.plan_name, 'free'),
            COALESCE(u.count, 0)::INTEGER
        ) as tiers
        FROM available_plans ap
        LEFT JOIN (
            SELECT 
...
-- Let's also check what plans are actually in the subscription_plans table
